...
void CWeaponMagazinedWGrenade::PlayAnimReload()
{
	...
	if(IsGrenadeLauncherAttached())
	{
		if(IsMisfire() && isHUDAnimationExist("anm_reload_jammed_w_gl"))
			PlayHUDMotion("anm_reload_jammed_w_gl", TRUE, this, GetState());
		else if (IsPartlyReloading())
			PlayHUDMotionIfExists({ "anm_reload_w_gl" }, true, GetState());
		else
		...
		else
		{
			...
			if(pActor)
			{
				...
				else if (State & mcAnyMove)
				{
					if (!(State & mcCrouch)) 
					{
						if (State & mcAccel) //Oiauaa iaaeaiiay (SHIFT)
							act_state = 5;
						else
							act_state = 2;
					}
					else if (State & mcAccel) //Oiauaa a i?enyaa (CTRL+SHIFT)
						act_state = 4;
					else
						act_state = 3;
				}
