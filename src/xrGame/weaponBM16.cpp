...
void CWeaponBM16::PlayAnimShoot()
{
	...
	strcat(guns_shoot_anm, std::string("anm_shoot" + (this->IsZoomed() && !this->IsRotatingToZoom()) ? "_aim_" : "_" + std::to_string(m_magazine.size())).c_str());
...
void CWeaponBM16::PlayAnimIdle()
{
	...
	if(IsZoomed())
	{
		if (IsRotatingToZoom())
		{
			...
			strcat(guns_aim_anm, ("anm_idle_aim_start_" + std::to_string(m_magazine.size())).c_str());
		...
		if (const char* guns_aim_anm = GetAnimAimName()) 
		{
			...
			strcat(guns_aim_anm_full, (std::string(guns_aim_anm) + "_" + std::to_string(m_magazine.size())).c_str());
	...
	else
	{
		...
		{
			string32 guns_aim_anm;
			strcat(guns_aim_anm, ("anm_idle_aim_end_" + std::to_string(m_magazine.size())).c_str());
			if (isHUDAnimationExist(guns_aim_anm)) 
			{
				PlayHUDMotionNew(guns_aim_anm, true, GetState());
				return;
